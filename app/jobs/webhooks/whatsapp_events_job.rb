class Webhooks::WhatsappEventsJob < ApplicationJob
  ...
  def perform(params = {})
    ...
    Rails.logger.info "WhatsappEventsJob: Started with params: #{params.keys}"
    channel = find_channel_from_whatsapp_business_payload(params)
    Rails.logger.info "WhatsappEventsJob: Channel found? #{channel.present?} (ID: #{channel&.id}, Provider: #{channel&.provider})"
    ...
    when 'wuzapi'
      Rails.logger.info 'WhatsappEventsJob: Triggering WuzAPI Service'
