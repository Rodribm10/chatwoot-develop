  ...
  class ScenarioDelegatorTool < Captain::Tools::BasePublicTool
    ...
    def perform(_tool_context, pergunta_interna: nil, **kwargs)
      ...
      if @conversation
        ...
        if reset_index
          ...
          recent_history = recent_history[(reset_index + 1)..-1] || []
          reset_detected = true
        end
        ...
        if last_user_msg.present? && !history_text.include?(last_user_msg)
    ...
    def extract_name_and_cpf(text)
      ...
      candidate_line = lines.find do |line|
        !blacklist_patterns.any? { |p| line.include?(p) } && !line.include?(':')
      end
